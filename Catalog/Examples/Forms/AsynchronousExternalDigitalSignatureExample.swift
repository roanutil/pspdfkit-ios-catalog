...
class AsynchronousExternalDigitalSignatureExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController? {
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            signer.sign(signatureFormElement, withCertificate: signCertificate, writeTo: destinationFilePath) { success, signedDocument, error in
                if success {
                    DispatchQueue.main.async {
                    ...
                    let alert = UIAlertController(title: "Error", message: "The document could not be signed. Error: \(error).", preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: "OK", style: .default))
                    delegate.currentViewController?.present(alert, animated: true)
                }
            }
            // Make sure these are not deallocated before the async closure exits.
            _ = unsignedDocument
            _ = signerDelegate
            _ = documentSignerDataSource
        }
        return nil
    }
}
